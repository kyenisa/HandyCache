		...
		if re.match(file_type_group, [[(Image|Style)]]) then
			...
			if size_code_num <= size_min or size_code_num >= size_max then
				...
				if size_code_num < size_min then
					table.insert(data.monitor_string_array, {priority=3, text='No saved: min limit is: '..size_min..' kB (file size: '..num_repack(size_code_num)..')'})
					set_color({153, 0, 0}, 2)
				else
					table.insert(data.monitor_string_array, {priority=3, text='No saved: max limit is: '..size_max..' kB (file size: '..num_repack(size_code_num)..')'})
					set_color({153, 0, 0}, 2)
				end
			end
		end
