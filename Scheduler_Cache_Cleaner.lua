--[[ <HCExtension>
@name			Scheduller - Cache Cleaner
@author			mai62, truefriend-cz (translate to English and formating code)
@version		0.61
@description	Cache Cleaner scheduler
...
function ini_file_name()
	...
	if not fn or (fn=='') then return [[c:\CacheCleaner.ini]] end
...
function Timer1m()
	...
	if hc_static['NextStart'] < t then
		...
		hc.put_msg('Cache Cleaner running')
	end
end
...
function Options()
	...
	Form._ = { Caption='Cache Cleaner - Settings', width=380, height=160, BorderStyle='Fixed3D' }
