...
@version		1.1
@description	Script for Average Loading data from internet and saving (must use with truefriend-cz script pack only) - Extension list position priority: 5
		...
		if re.match(file_type_group, [[(Image|Icon|Style|Javascript|Font)]], 1) then
			...
			if file_exists(afs_path_file) then
				...
				if file_content < limit then
					...
					code = file_content+1
					table.insert(data.monitor_string_array, {priority=400, text='Avg: '..code..'/'..limit..' hits'})
